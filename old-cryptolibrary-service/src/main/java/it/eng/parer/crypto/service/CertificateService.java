...
public class CertificateService {
    ...
    /**
     * Ottieni l'authKeyId del certificato del firmatario. Questo metodo effettua la decodifica tramite BouncyCastle
     * dello stream di dati codificati DER.
     *
     * @param extvalue
     *            byte array del ceritificato del firmatario.
     *
    ...
    public String getCertificateKeyId(byte[] extvalue) {
        try {
            ...
            X509Certificate x509Cert = (X509Certificate) fact.generateCertificate(new ByteArrayInputStream(extvalue));
            return SignerUtil.getAuthorityKeyId(x509Cert);

        } catch (NoSuchProviderException | IOException | CertificateException ex) {
            throw new CryptoParerException().withCode(ParerError.ErrorCode.CERT_IO).withMessage(ex.getMessage())
                    .withDetail("Errore di INPUT/OUTPUT, sul certificato");
        }
    }
    ...
    public ParerCertificate addCaCertificate(byte[] derCertificate) {
        ...
        } catch (IOException | CertificateException | NoSuchProviderException | CryptoStorageException ex) {
            throw new CryptoParerException().withCode(ParerError.ErrorCode.CERT_EXCEPTION).withMessage(ex.getMessage())
                    .withDetail("Errore durante l'inserimento del certificato");
        }
    }
     ...
     *            id della CA
     *
     * @return vero se il certificato esiste in banca dati.Falso altrimenti. Non viene effettuato alcun controllo di
     *         validità temporale.
     */
    public boolean existsCaCertificate(String subjectDN, String keyId) {
        try {
            ...
            throw new CryptoParerException().withCode(ParerError.ErrorCode.CERT_EXCEPTION).withMessage(ex.getMessage())
                    .withDetail("Errore durante il recupero del certificato");
        }

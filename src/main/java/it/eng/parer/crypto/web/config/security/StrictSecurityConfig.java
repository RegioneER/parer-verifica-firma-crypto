 ...
 * Valida se disattivata la console web dell'admin (parer.crypto.admin-ui.enabled = false).
 * Le regole su URI pattern match definiscono la "catena" dei permessi, quindi l'ordine è importante.
 */
@Configuration
@ConditionalOnProperty(name = "parer.crypto.admin-ui.enabled", havingValue = "false")
public class StrictSecurityConfig {
    ...
    protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable()) // disable csrf
                ...
                .authorizeHttpRequests(authorizeHttpRequests -> authorizeHttpRequests
                        .requestMatchers(new AntPathRequestMatcher(URL_ADMIN_BASE + "/**")).denyAll()) // deny admin

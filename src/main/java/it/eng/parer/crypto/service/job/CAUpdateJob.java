...
public class CAUpdateJob {
    ...
    private void loadingCa() {
        try {
            ...
            Collection<X509Certificate> qualifiedCertificate = signerUtil
                    .getQualifiedPrincipalsAndX509Certificates().values();
            final int size = qualifiedCertificate.size();
            log.atInfo().log("Trovati {} certificati da eIDAS", size);
            // Utilizzo l'iteratore e rimovo l'elemento per rendere eleggibile al GC il record già
            // processato
    ...
    private List<String> getCrlDistributionUrls(X509Certificate cert, String subjectKeyId) {
        ...
        try {
            ...
            log.atWarn().log(
                    "Non è stato possibile recuperare la CRL dal distribution point - Subject Key ID: {}",
                    subjectKeyId);
        }
        return urls;
    }
    ...
    private void saveCRL(List<String> distributionPoints) throws CryptoStorageException {
        if (distributionPoints != null) {
            X509CRL crl = signerUtil.getCrlByURL(distributionPoints, httpClientTimeout,

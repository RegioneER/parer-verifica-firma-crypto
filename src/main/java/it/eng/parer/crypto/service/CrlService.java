...
public class CrlService {
     ...
     *
     * @param urls lista di url da cui scaricare la nuova crl.
     *
     * @return Modello del Parer per le crl.
     */
    public ParerCRL addCrlByURL(List<String> urls) {
        try {
            X509CRL cryptoCrl = signerUtil.getCrlByURL(urls, httpClientTimeout,
                    httpClientSocketTimeout, ldpaClientTimeout);
            if (cryptoCrl == null) {
            ...
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_CRYPTO_STORAGE).withMessage(
                    "Errore nel recupero e/o inserimento della CRL da/sul DB (url input: "
                            + String.join(",", urls) + ")");
        } catch (IOException ex) {
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_IO)
                    .withMessage("Errore durante il parsing del numero di CRL (url input: "
                            + String.join(",", urls) + ")");
        }
    }
     ...
     *
     * @param subjectDN Distinguished Name della CA che emette la CRL.
     * @param keyId     Authority Key Identifier (in hex) del certificato.
     *
     * @return Modello del Parer per le crl.
     */
    public ParerCRL getCrl(String subjectDN, String keyId) {
        try {
            ...
            if (cryptoCrl == null) {
                throw new CryptoParerException().withCode(ErrorCode.CRL_NOT_FOUND)
                        .withMessage("CRL non trovata (subjectDN: [" + subjectDN + "] keyId: ["
                                + keyId + "])")
                        .withDetail(
            ...
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_CRYPTO_STORAGE).withMessage(
                    "Errore nel recupero e/o inserimento della CRL da/sul DB (subjectDN: ["
                            + subjectDN + "] keyId: [" + keyId + "])");
        } catch (CRLException ex) {
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_EXCEPTION).withMessage(
                    "CRL encoding error (subjectDN: [" + subjectDN + "] keyId: [" + keyId + "])");
        } catch (IOException ex) {
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_IO)
                    .withMessage("Errore durante il parsing del numero di CRL (subjectDN: ["
                            + subjectDN + "] keyId: [" + keyId + "])");
        }
    }
     ...
     * Ottieni la ParerCRL.
     *
     * @param uniqueId MD5 di subjectdn + keyId
     *
     * @return ParerCRL
     */
    public ParerCRL getCRL(String uniqueId) {
        try {
            ...
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_CRYPTO_STORAGE).withMessage(
                    "Errore nel recupero e/o inserimento della CRL da/sul DB (uniqueId: ["
                            + uniqueId + "])");
        } catch (CRLException ex) {
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_EXCEPTION)
            ...
            throw new CryptoParerException(ex).withCode(ErrorCode.CRL_IO).withMessage(
                    "Errore durante il parsing del numero di CRL  (uniqueId: [" + uniqueId + "])");
        }
    }
